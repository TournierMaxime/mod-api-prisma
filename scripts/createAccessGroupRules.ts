...
await prismaService.prisma.$transaction(async (prisma: any) => {
  const [superAdminGroup] = await prismaService.findOrCreate(
    prisma.accessGroups,
    { name: "superAdmin" },
    { name: "superAdmin" }
  )
  ...
  const [adminGroup] = await prismaService.findOrCreate(
    prisma.accessGroups,
    { name: "admin" },
    { name: "admin" }
  )
  ...
  const [customerGroup] = await prismaService.findOrCreate(
    prisma.accessGroups,
    { name: "customer" },
    { name: "customer" }
  )
